...
def _export(**overrides) -> ExportRequest:
    defaults = dict(
        question="What BLEU score does the Transformer achieve?",
        answer="The Transformer achieves 28.4 BLEU on English-to-German [S1].",
        citations=[
            CitationSchema(
                chunk_id="c1",
                document_id="doc123",
                ...
                snippet="On the WMT 2014 English-to-German translation task...",
            )
        ],
...
def test_markdown_includes_question_answer_and_citation_link():
    ...
    assert "What BLEU score does the Transformer achieve?" in md
    assert "28.4 BLEU" in md
    assert "attention_is_all_you_need.pdf, page 8" in md
    assert "http://localhost:8000/api/documents/doc123/file#page=8" in md
    assert "On the WMT 2014" in md
